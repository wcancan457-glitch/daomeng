...
echo [install] DaoMeng one-click install
...
if "%DAOMENG_SKIP_FRONTEND_BUILD%"=="1" (
  echo [install] Skipping frontend build because DAOMENG_SKIP_FRONTEND_BUILD=1.
) else (
  npm run build
)

echo.
echo [install] DaoMeng installation complete.
echo.
echo Next steps:
