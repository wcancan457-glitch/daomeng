#!/usr/bin/env bash
# One-click installer for DaoMeng.
...
install_frontend() {
  ...
  if [ "${DAOMENG_SKIP_FRONTEND_BUILD:-0}" = "1" ]; then
    warn "Skipping frontend build because DAOMENG_SKIP_FRONTEND_BUILD=1."
  else
    npm run build
  fi
}
...
main() {
  info "DaoMeng one-click install"
  check_python
  check_ffmpeg
  install_backend
  install_frontend
